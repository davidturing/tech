...
本章将深入解析“左脑（The Left Brain）”的内部构造。我们将探讨如何利用 Google Gemini 3.1 Pro 的原生结构化能力，结合 Python 的 Pydantic 数据契约，将互联网上嘈杂、非结构化的信息流，锻造为绝对严谨、可追溯、可持久化的本地知识图谱（PageIndex），并揭秘左脑作为“系统免疫系统”进行事实核查（Fact-Checking）的核心机制。
...
#### 3.1.1 物理基座：Gemini 3.1 Pro

我们选择 Gemini 3.1 Pro 作为左脑的物理基座，而非其他模型，主要基于以下三个工程维度的考量：
...
2. **超长上下文窗口**：在处理长篇研报或几十条堆叠的 X 推文时，左脑需要极强的上下文召回能力（Needle In A Haystack）来提取实体关系，Gemini 3.1 Pro 在这方面表现出了卓越的稳定性。
...
- **[[qwen3-max-2026-01-23]]** (Model): 阿里云发布的最新旗舰推理模型。
- **[[Gemini 3.1 Pro]]** (Model): Google 发布的具备长文本与强结构化输出能力的大模型。

## 逻辑关系 (Triples)
- [[OpenClaw]] == 适合于 ==> [[I/O 密集型任务]] *(补充: 得益于 Node.js 的底层特性)*
- [[qwen3-max-2026-01-23]] == 担任 ==> [[主控大脑]] *(补充: 负责全局调度与创意)*
- [[Gemini 3.1 Pro]] == 挂载为 ==> [[外部 Tool]] *(补充: 专门负责事实提取与多模态解析)*
...
![红蓝对抗事实核查 (Red-Blue Teaming)](../images/06_leftbrain_redblue_teaming_1771650271440.png)
